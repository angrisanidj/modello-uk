...
const CONFIG = {
  ...
  gbSeats: 632,
  niSeats: 18,
  cacheVersion: 'uk-v0912-20260826-ref-latent-potential',
  swingLambda: 0.82,
...
function reformDiagnosticsReady(){
  const m=state.mrpLite;
  return m?.version==='uk-v0912-ref-latent-potential-live'
    && m?.status==='ok'
    && m?.shadow_only===true
    && m?.approved===false;
}
...
function productionModelLabel(){
  const diagnostic=reformDiagnosticsReady()?' · v0.9.12 potenziale Reform latente in shadow':'';
