...
const CONFIG = {
  ...
  gbSeats: 632,
  niSeats: 18,
  cacheVersion: 'uk-v0920-20260827-scotland-conlab',
  swingLambda: 0.82,
...
function shadowDiagnosticsReady(){
  const m=state.mrpLite;
  return m?.version==='uk-v0920-scotland-conlab-sweep-live'
    && m?.status==='ok'
    && m?.shadow_only===true
    && m?.approved===false;
}
...
function productionModelLabel(){
  const diagnostic=shadowDiagnosticsReady()?' · v0.9.20 sweep Scozia + Con/Lab in shadow':'';
