...
const CONFIG = {
  ...
  gbSeats: 632,
  niSeats: 18,
  cacheVersion: 'uk-v0916-20260827-local-strength-refinement',
  swingLambda: 0.82,
...
function reformDiagnosticsReady(){
  const m=state.mrpLite;
  return m?.version==='uk-v0916-local-strength-refinement-live'
    && m?.status==='ok'
    && m?.shadow_only===true
    && m?.approved===false;
}
...
function productionModelLabel(){
  const diagnostic=reformDiagnosticsReady()?' · v0.9.16 local refinement in shadow':'';
