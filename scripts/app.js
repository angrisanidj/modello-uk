...
const CONFIG = {
  ...
  gbSeats: 632,
  niSeats: 18,
  cacheVersion: 'uk-v0913-20260826-reform-structural-sweep',
  swingLambda: 0.82,
...
function reformDiagnosticsReady(){
  const m=state.mrpLite;
  return m?.version==='uk-v0913-reform-structural-sweep-live'
    && m?.status==='ok'
    && m?.shadow_only===true
    && m?.approved===false;
}
...
function productionModelLabel(){
  const diagnostic=reformDiagnosticsReady()?' · v0.9.13 sweep strutturale Reform in shadow':'';
